...
export default function ChatPage() {
  ...
  const createPeerConnection = (
    ...
    const currentSocketId = activeSocket.id;
    if (!currentSocketId) {
      console.error('[WebRTC] Cannot create peer connection: activeSocket.id is undefined');
      return null;
    }
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate && activeSocket.id) {
    ...
    const isPolite = currentSocketId < targetSocketId;
  ...
  const startCall = async (withVideo: boolean) => {
    if (!socket || !socket.id || !currentRoom) return;
  ...
  const endCall = () => {
    if (socket && socket.id && currentRoom) {
